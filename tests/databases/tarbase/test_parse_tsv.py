...
@pytest.mark.tarbase
def test_tarbase_tsv_parse_virus():
    """
    Try to parse the interaction for a viral species
    """
    ...
    related_sequences = [
        RelatedSequence(
            sequence_id=f"ENSEMBL:ENSMUSG00000000740",
            relationship="target_protein",
            evidence=RelatedEvidence(methods=["qCLASH"]),
        )
    ]
    ## NB: the method changed between version 8 & 9: Microarrays -> PAR-CLIP
    url = "https://dianalab.e-ce.uth.gr/tarbasev9/interactions?gene=Rpl13&mirna=mghv-miR-M1-1-3p"
    correct_entry = Entry(
        primary_id="mghv-miR-M1-1-3p",
        accession="TARBASE:mghv-miR-M1-1-3p",
        ncbi_tax_id=33708,
        database="TARBASE",
        sequence="TAGAAATGGCCGTACTTCCTTT",
        regions=[],  ## Don't create regions for protein target
        rna_type="SO:0000276",
        url=url,
        seq_version=1,
        optional_id="MIMAT0001564",
        description="Murid gammaherpesvirus 4 (Murine herpesvirus 68) mghv-miR-M1-1-3p",
        note_data={"url": url},
        xref_data={},
        related_sequences=related_sequences,
    )

    parsed_entry = parse("data/tarbase/tarbase_viral.tsv")[0]

    assert parsed_entry == correct_entry


@pytest.mark.tarbase
def test_tarbase_tsv_missing_sepcies():
    """
    Try to parse the interaction for a viral species that is not in the taxonomy
    """
    ...
    parsed_entry = parse("data/tarbase/tarbase_viral_missing.tsv")[0]
