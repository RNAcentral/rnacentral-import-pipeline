...
class RfamHit(object):
    ...
    target_name = attr.ib()
    seq_acc = attr.ib(converter=dash_as_none())
    rfam_name = attr.ib()
    rfam_acc = attr.ib()
    mdl = attr.ib()
    mdl_from = attr.ib(converter=as_0_based)
    mdl_to = attr.ib(converter=int)
    seq_from = attr.ib(converter=as_0_based)
    seq_to = attr.ib(converter=int)
    strand = attr.ib(converter=convert_strand)
    trunc = attr.ib(converter=convert_trunc)
    infernal_pass = attr.ib(converter=int)
    infernal_gc = attr.ib(converter=float)
    bias = attr.ib(converter=float)
    score = attr.ib(converter=float)
    e_value = attr.ib(converter=float)
    inc = attr.ib()
    description = attr.ib(converter=dash_as_none())
...
class RfamClanHit(object):
    ...
    idx = attr.ib(converter=as_0_based)
    rfam_name = attr.ib()
    rfam_acc = attr.ib()
    seq_name = attr.ib()
    seq_acc = attr.ib(converter=dash_as_none())
    clan_name = attr.ib(converter=dash_as_none())
    mdl = attr.ib()
    mdl_from = attr.ib(converter=as_0_based)
    mdl_to = attr.ib(converter=int)
    seq_from = attr.ib(converter=as_0_based)
    ...
    e_value = attr.ib(converter=float)
    inc = attr.ib()
    overlap = attr.ib(converter=convert_overlap)
    anyidx = attr.ib(converter=dash_as_none(as_0_based))
    afrct1 = attr.ib(converter=dash_as_none(float))
    afrct2 = attr.ib(converter=dash_as_none(float))
    winidx = attr.ib(converter=dash_as_none())
    wfrct1 = attr.ib(converter=dash_as_none())
    wfrct2 = attr.ib(converter=dash_as_none())
    description = attr.ib(converter=dash_as_none())
