...
def parse(data: pathlib.Path) -> ty.Iterable[Entry]:
    ...
    species_info["Species"] = species_info["Species"].apply(
    ...
    total_entries = len(
        gff_db.execute(
            "select DISTINCT(id) from features where featuretype = 'transcript' "
        ).fetchall()
    )
    entries = []
    for gene_id_q in tqdm(
        gff_db.execute("select id from features"), total=total_entries
    ):
        ...
        chromosome = fasta_db[
            gff_db[primary_id].seqid
        ]  ##Hopefully gets the right chromosome?
        ...
        entries.append(
            Entry(
                ...
                description=_generate_description(
                    int(taxid), gene_info["Gene_ID"].values[0]
                ),
