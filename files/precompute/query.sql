...
FROM rna
JOIN :tablename todo ON todo.urs = rna.urs
JOIN xref
ON
    xref.urs = rna.urs
...
ON
    region.urs_taxid = xref.urs_taxid
...
on
  ss.urs = rna.urs
  and COALESCE(ss.assigned_should_show, ss.inferred_should_show)
