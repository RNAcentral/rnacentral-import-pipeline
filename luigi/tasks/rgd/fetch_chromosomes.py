...
class RgdFetchChromosomes(luigi.Task):
    organism = luigi.Parameter()

    @property
    def remote_files(self):
        if not hasattr('__chromosome_files'):
            self.__chromosome_files = helpers.chromosome_files(rgd().host, self.organism)
        return self.__chromosome_files
