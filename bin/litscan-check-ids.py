...
nts = re.compile("^[acgu]+$")
numbers_and_dash = re.compile(r"^\d+[\-]\d+$")  # do not use ids like 6-1, 260-1, etc
...
def check_id(item):
    if item.isnumeric() or item.lower() in words or numbers_and_dash.search(item):
        ...
        len(item) > 2
        and not special_char.search(item)
        and not nts.search(item.lower())
        and "\\" not in item
    ):
        result = item
    else:
