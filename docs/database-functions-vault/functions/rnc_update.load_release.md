...
or **delta** path depending on the release type, marks the release done, and logs end.
...
- **Calls (in order):**
  ...
  - `rnc_load_xref_incremental.load_xref_delta` *(when type = `D`)* — see [[external-dependencies]]
